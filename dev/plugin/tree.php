...
class tree
{
    protected ?\ounun\db\pdo $db;
    protected $table;
    protected $field_id;
    protected $field_parent_id;
    protected $field_name;

    function __construct($table, $id = 'id', $parent_id = 'parent_id', $name = 'name')
    {
        $this->table           = $table;
        $this->field_id        = $id;
    ...
    function set($data)
    {
        if (isset($data[$this->field_parent_id]) && intval($data[$this->field_parent_id]) < 1) {
            $data[$this->field_parent_id] = null;
        }
        ...
        if (isset($data[$this->field_id]) && $data[$this->field_id]) {
            ...
            if (!$r) return false;
            $is_update = 0;
            if (array_key_exists($this->field_parent_id, $data) && $r[$this->field_parent_id] !== $data[$this->field_parent_id]) {
                if (isset($data[$this->field_parent_id])
                    && in_array($data[$this->field_parent_id], explode(',', $r['childids']))) {
                    $this->error = '逻辑错误';
                    return false;
                } else {
                    $is_update = 1;
            ...
            if ($is_update) {
                ...
                if (array_key_exists($this->field_parent_id, $data)) {
            ...
            if (isset($data[$this->field_parent_id]) && $data[$this->field_parent_id] > 0) {
    ...
    function rm($id)
    {
        ...
        if (!is_null($r[$this->field_parent_id])) {
    ...
    {
        if (is_null($id)) {
            $data = $this->db->select("SELECT $fields FROM `$this->table` WHERE `$this->field_parent_id` is null AND $where ORDER BY `sort`");
        } else {
            $data = $this->db->select("SELECT $fields FROM `$this->table` WHERE `$this->field_parent_id`=? AND $where ORDER BY `sort`", array($id));
        }
        if ($deep && $data) {
            foreach ($data as $k => $r) {
    ...
    function get_parent($id, $fields = '*', $is_start = 1)
    {
        ...
        if (!is_null($r[$this->field_parent_id])) {
            $data = array_merge($data, $this->get_parent($r[$this->field_parent_id], $fields, 0));
        }
