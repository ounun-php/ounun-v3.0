...
class html
{
    ...
    public function __construct($cache_config, string $key = '', int $expire = 3600, bool $trim = true, bool $debug = true)
    {
        ...
        // Cache
        $this->_cache_driver = new cache($cache_config, $this->_is_debug);
        $this->_cache_driver->key($key);
    }
    ...
    public function run_cache_check()
    {
        ...
        if ($this->_cache_time + $this->_expire > $this->_now_time) {
            \ounun\debug::header('xypc',$this->_cache_driver->filename(),$this->_is_debug,__FUNCTION__,__LINE__);
            return true;
        }
        $cache_time_t = $this->_cache_driver->cache_time_tmp();
        ...
        if ($cache_time_t + self::Cache_Time_Interval > $this->_now_time) {
            \ounun\debug::header('xypc_t',$this->_cache_driver->filename().'.t time:'.$cache_time_t,$this->_is_debug,__FUNCTION__,__LINE__);
            return true;
        }
        $this->_cache_time = 0;
        return false;
    ...
    {
        \ounun\debug::header('xypm',$this->_cache_driver->filename(),$this->_is_debug,__FUNCTION__,__LINE__);
        $this->stop = false;
        $this->_cache_driver->cache_time_tmp_set();
        // 生成
        // ob_start();
        template::ob_start();
    ...
    public function run_output()
    {
        if ($this->_cache_time) {
            ...
            header('Etag: ' . $etag);
            // 输出
            $this->_cache_driver->cache_out($this->_gzip);
        }
