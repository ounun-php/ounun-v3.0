...
class json
{
    ...
    function encode($arg)
    {
        ...
        if (function_exists('json_encode')) return json_encode($arg);
        $returnValue = '';
        $c           = '';
        $i           = '';
        $l           = '';
    ...
    function decode($text, $assoc = true)
    {
        if (empty($text))
        {
            return '';
        ...
        else
        {
           ...
           $this->next();
           $return = $this->val();
           $result = $assoc ? (array) $return : $return;
        }
    	if ($this->charset != 'utf-8') $result = str_charset('utf-8', $this->charset, $result);
        return $result;
    }
