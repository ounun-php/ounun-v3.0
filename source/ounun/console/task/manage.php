...
class manage
{
    ...
    /** @var string 采集 - 运行类型 */
    const Base_Type_Caiji  = 'caiji';
    /** @var string 发布 - 运行类型 */
    const Base_Type_Post   = 'post';
    /** @var string 系统 - 运行类型 */
    const Base_Type_System = 'system';
    /** @var string 站点 - 运行类型 */
    const Base_Type_Site   = 'site';
    /** @var string 总后台管理(采集) - 运行类型 */
    const Base_Type_Biz    = 'biz';
    /** @var array 运行类型(采集/发布/系统) */
    const Base_Types   = [
        self::Base_Type_Caiji  => '采集',
        self::Base_Type_Post   => '发布',
        self::Base_Type_System => '系统',

        self::Base_Type_Site   => '站点',
        self::Base_Type_Biz    => '采集(总后台)',
    ];
     ...
     * @return manage
     */
    public static function i(string $tag , pdo $db_biz = null, pdo $db_caiji = null, pdo $db_site = null, string $tag_db_logs = 'caiji', string $tag_db_task = 'caiji'): self
    {
    ...
    public static function db_biz(array $db_config = [])
    {
        if (empty(static::$_db_biz)) {
            static::$_db_biz = pdo::i('biz', $db_config);
        }
    ...
    public static function db_caiji(array $db_config = [])
    {
        if (empty(static::$_db_caiji)) {
            static::$_db_caiji = pdo::i(static::Base_Type_Caiji, $db_config);
        }
    ...
    public static function db_site(array $db_config = [])
    {
        if (empty(static::$_db_site)) {
            static::$_db_site = pdo::i(static::Base_Type_Site, $db_config);
        }
    ...
    {
        if(static::Base_Type_Caiji == static::$_tag_db_logs){
            return static::db_caiji();
        }elseif(static::Base_Type_Biz == static::$_tag_db_logs){
            return static::db_biz();
        }elseif(static::Base_Type_Site == static::$_tag_db_logs){
    ...
    {
        if(static::Base_Type_Caiji == static::$_tag_db_task){
            return static::db_caiji();
        }elseif(static::Base_Type_Biz == static::$_tag_db_task){
            return static::db_biz();
        }elseif(static::Base_Type_Site == static::$_tag_db_task){
            return static::db_site();
        }
        return static::db_caiji();
    }
     ...
     * `extend` text NOT NULL COMMENT '任务参数paras/扩展json',
     * PRIMARY KEY (`id`),
     * KEY `state` (`state`),
     * KEY `cls` (`tag`,`tag_sub`)
