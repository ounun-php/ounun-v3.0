...
class html
{
    ...
    public function __construct($cache_config, string $key = '', int $expire = 3600, bool $trim = true, bool $debug = true)
    {
        ...
        // Cache
        $this->_cache = new cache($cache_config, $this->_is_debug);
        $this->_cache->key($key);
    }
    ...
    public function callback(bool $output)
    {
        ...
        ob_implicit_flush(1);
        // 写文件
        \ounun\debug::header( $filesize, 'xypm_size',$this->_is_debug, __FUNCTION__, __LINE__);
        if ($filesize > self::Cache_Mini_Size) {
            \ounun\debug::header( $this->_cache->filename(), 'xypm_ok',$this->_is_debug, __FUNCTION__, __LINE__);
            ...
            $this->_cache->delete();
            debug::header( 'nocache', 'xypm_noc', __FUNCTION__, __LINE__);
            if ($output) {
